...
def REF(S, N=1):          #对序列整体下移动N,返回序列(shift后会产生NAN)    
    if isinstance(N, (int, float)):
        ...
        for i in range(len(S)):
            n = N[i]
            # 动态位移仅在 0<=n<=i 时有效，其他位置按通达信语义返回空值
            if (not np.isnan(n)) and (n >= 0) and (n <= i):
                n = int(n)
                res[i] = S[i - n]
        return res
...
def HHV(S, N):  #HHV,支持N为序列版本
    ...
    if isinstance(N, (int, float)):
        ...
        for i in range(len(S)):
            n = N[i]
            # 动态窗口长度必须 >=1，避免出现空切片导致 max/min 报错
            if (not np.isnan(n)) and (n >= 1) and (n <= i + 1):
                n = int(n)
                res[i] = S[i + 1 - n:i + 1].max()
        return res
